...
public class ResourceResolver implements Subscriber {
    ...
    private ResourceResolver() {
        Application.getSubscriptionManager().subscribe(this, Application.LOAD_RESOURCES, LOAD_RESOURCES_FROM_PLUGIN_JAR);
    }
                ...
                try {
                    ...
                    while (itr.hasMoreElements()) {
                        ...
                        if ((value = getPluginRequiredResource(resourceName, resList)) != null) {
                            System.out.println("[INFO] => Found Reosurce :" + resourceName + " in :" + file.getAbsolutePath());
                            File extracted = this.extractArchiveEntry(entry, jarFile);
                            if (extracted != null) {
                                this.resourceMap.put(resourceName, extracted.getAbsolutePath());
